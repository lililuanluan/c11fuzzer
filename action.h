...
public:
	...
	void init_position_hash() {
		// model_print(("value = %llu, type = %s\n"), value, get_type_str());
		// getchar();
		if (position != nullptr) {
			// std::hash is non-deterministic, use another one
			position_hash = hash_djb2(position);
			// model_print("hash: %#llx\n", position_hash);
			// getchar();
#ifdef COMB_POS_WITH_TID
			position_hash += tid;
#endif
		}
		if (position) {
		...
		// if (this->is_write()) {
		// 	position_hash = (position_hash << 32) ^ value;
		// }
	}

	uint64_t get_position_hash() const { return position_hash; }
