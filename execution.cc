...
void ModelExecution::process_write(ModelAction* curr)
{
	...
	for (auto&& rf : replayer.rf_map) {
		if (curr->get_position_hash() == rf.second) {
			model_print("current write %llu is in rf map\n", curr->get_position_hash());
			// getchar();
			write_flag = true;
		}
	}
...
{
#ifdef REPLAY
	int curr_threadid = id_to_int(curr->get_tid());
	Thread* curr_thread = get_thread(curr);
	const char* type_str = curr->get_type_str();
	const char* mo_str = curr->get_mo_str();
	// model_print(BLUE("\n current action type is  %-14s. on thread %d, sequence number is : %d , mo_type is : %7s. hash : %llu, is_read = %s\n"), type_str, curr_threadid, curr->get_seq_number(), mo_str, curr->get_position_hash(), curr->is_read() ? "true" : "false");
#endif
	ASSERT(curr);
...
#ifdef NEW_HASH
static unsigned int hash_execution(action_list_t* list) {
	unsigned int hash = 0;
	std::map <thread_id_t, unsigned int> hash_by_tid;
	// std::unordered_map<thread_id_t, unsigned int> prev_hash;
	for (auto it = list->begin(); it != NULL; it = it->getNext()) {
		const ModelAction* act = it->getVal();
		if (act->get_seq_number() > 0) {
			auto tid = act->get_tid();
			auto& h = hash_by_tid[tid];
			auto act_hash = it->getVal()->hash();
			// if (prev_hash[tid] != act_hash) {
			h = h ^ (h << 3) ^ (act_hash);
			// }
			// prev_hash[tid] = act_hash;
		}
	}
	// combine hash
	model_print("size of hash_by_tid: %u\n", hash_by_tid.size());
	// getchar();
	// std::vector<unsigned int> hash_by_tid_v;
	for (const auto& h : hash_by_tid) {
		// hash_by_tid_v.push_back(h.second);
		hash = hash ^ (hash << 3) ^ (h.second);
	}
	// std::sort(hash_by_tid_v.begin(), hash_by_tid_v.end());
	// for (const auto& h : hash_by_tid_v) {
	// 	hash = hash ^ (hash << 3) ^ (h);
	// }
	return hash;
}
...
Thread* ModelExecution::action_select_next_thread(const ModelAction* curr) const
{
	...
	if (write_flag == true) {
		write_flag = false;
		model_print("selecting next thread by action\n");
		// getchar();
