...
Thread* ModelChecker::get_next_thread()
{
	 ...
	 */
#ifdef REPLAY
	// model_print("model get_next_thread, calling scheduler select_next_thread\n");
	// GETCHAR
#endif
...
{
#ifdef REPLAY
	// model_print("model getNextThread, get_num_threads() = %u\n", get_num_threads());
	// GETCHAR
#endif
	...
	if (nextThread != nullptr) {
		// model_print("model getNextThread finished, got %d\n", id_to_int(nextThread->get_id()));
	}
	else {
		// model_print("model getNextThread NULL\n");
	}
	// GETCHAR
#endif
	return nextThread;
...
{
#ifdef REPLAY
	// model_print("model handle chose thread ");
	// if (old != nullptr) model_print("%d", id_to_int(old->get_id()));
	// model_print("\n");
#endif
...
void ModelChecker::startChecker() {
	...
	// luan: set random seeds
	// srand(time(nullptr));
	model_print("start checker\n");
	auto seed = getenv("SEED");
	if (seed != nullptr) {
		auto seed_v = std::stoul({ seed });
		model_print("seed = %s, to unsigned = %u\n", seed, seed_v);
		srand(seed_v);
		// getchar();
	} else {
		srand((time(NULL) & 0xFFFF) | (getpid() << 16));	// trying to get a better seed
	}
	
	// initstate(423121, random_state, sizeof(random_state));
	initstate(rand(), random_state, sizeof(random_state));
